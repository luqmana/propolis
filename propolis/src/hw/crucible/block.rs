...
impl<R: BlockReq> CrucibleBlockDev<R> {
    ...
    pub fn from_options(
        ...
        match runtime {
            Some(runtime) => {
                runtime.spawn(task);
                println!(
                    "Crucible runtime spawned from CrucibleBlockDev::from_options"
                );
            }
            None => {
                tokio::spawn(task);
    ...
    fn process_rw_request(
        ...
        // XXX: this currently never fires, but it may in the future
        assert!((offset % self.block_size as usize) == 0);
        assert!((total_size % self.block_size as usize) == 0);
